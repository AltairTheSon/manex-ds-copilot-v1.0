...
if [ ! -d "dist/manex-ds-copilot/browser" ]; then
    echo "❌ Build directory structure is incorrect"
    exit 1
fi

# Check if index.html exists
if [ ! -f "dist/manex-ds-copilot/browser/index.html" ]; then
    echo "❌ index.html not found in build output"
    exit 1
fi

# Check if main JS files exist
if [ ! -f dist/manex-ds-copilot/browser/main*.js ]; then
    echo "❌ Main JavaScript bundle not found"
    exit 1
fi

# Check build size (should be under 5MB for Netlify)
BUILD_SIZE=$(du -sh dist/manex-ds-copilot/browser | cut -f1)
